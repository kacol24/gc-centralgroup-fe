...
export default function PropertyFinder({ compact = false }: PropertyFinderProps) {
  ...
  const queryVariables = useMemo(() => ({ lang: locale }), [locale]);
  ...
  const propertyTypes = useMemo(() => {
    return (
      propertyTypesResponse?.propertytypes?.map((propertyType) => ({
  ...
  return (
    ...
    >
      ...
      <button
        className="w-full bg-primary py-4 px-[92px] rounded-sm mt-0 text-xs font-semibold"
